"""
HIGH-CONFIDENCE VERIFICATION (close the miles/minutes gap)
=============================================================
Routes the 75 "high confidence" mismatches (margin > 0.15mi by straight-
line distance) to their candidate closer school, so every mismatch on
the map -- not just the close calls -- has a real walking-time number
instead of a straight-line mileage estimate.

USAGE:
  Place this script in the same folder as high_conf_verify_queue.json
  Run: python3 verify_high_confidence.py
  Outputs: high_conf_verify_results.json -- upload to Claude
"""
...
with open("high_conf_verify_queue.json") as f:
    samples = json.load(f)
...
results = []
total = len(samples)
print("Routing " + str(total) + " high-confidence addresses...\n")
for i, s in enumerate(samples):
    try:
        ...
        diff = (s.get("existing_walk_to_assigned") or 0) - adj
        print("[" + str(i+1).rjust(2) + "/" + str(total) + "] " + s["street"] + " " + str(s.get("num","")) +
              " -> " + s["destination_school"] + ": " + str(adj) + " min (vs " + str(s.get("existing_walk_to_assigned")) + " min assigned, diff=" + str(diff) + ")")
    except Exception as e:
