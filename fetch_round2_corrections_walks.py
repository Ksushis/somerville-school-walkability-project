"""
Walking distances for the 36 round-2 corrected sample points.
Run: python3 fetch_round2_corrections_walks.py
Outputs: round2_corrections_walks.json -- upload to Claude
"""
...
samples = []
with open("round2_corrections_geocodio_3fdab155f8d1a23d364ac51a61eca81fcc3d89ef.csv") as f:
...
for i, s in enumerate(samples):
    ...
    try:
        ...
        print("[" + str(i+1).rjust(2) + "/" + str(total) + "] " + s["street"] + " " + s["num"] + ": " + str(dist) + " mi (" + str(adj) + " min)")
    except Exception as e:
        print("[" + str(i+1).rjust(2) + "/" + str(total) + "] " + s["street"] + " " + s["num"] + ": ERROR " + str(e))
        results.append({**s, "walk_mi": None, "walk_min": None, "walk_min_adj": None})
    time.sleep(2)

with open("round2_corrections_walks.json", "w") as f:
    json.dump(results, f, indent=2)

failed = [r for r in results if r["walk_mi"] is None]
print("\nDone. Total: " + str(len(results)) + ", Failed: " + str(len(failed)))
print("Upload round2_corrections_walks.json to Claude")
