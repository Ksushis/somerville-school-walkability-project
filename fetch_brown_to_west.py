"""
Walking distances from all 268 Brown K-5 sample addresses to West Somerville School.
Run: python3 fetch_brown_to_west.py
Outputs: samples_walk_to_west.json — upload to Claude
"""
...
WEST = [-71.126467, 42.406166]  # 177 Powder House Blvd — geocod.io rooftop
...
def ors_walk(lon, lat, retries=3):
    ...
    body = json.dumps({"coordinates": [[lon, lat], WEST], "units": "mi"}).encode()
...
results = []
print("Routing " + str(len(samples)) + " addresses to West Somerville School...\n")
...
with open("samples_walk_to_west.json", "w") as f:
    json.dump(results, f, indent=2)

failed = [r for r in results if r["walk_mi"] is None]
print("\nDone. Failed: " + str(len(failed)))
print("Upload samples_walk_to_west.json to Claude")
