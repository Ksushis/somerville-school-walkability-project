"""
Walking distances for the 352 high-resolution Winter Hill zone sample
points (replacing the old single-point-per-street sampling).

Run: python3 fetch_winterhill_resample_walks.py
Outputs: winterhill_resample_walks.json -- upload to Claude
"""
...
samples = []
with open("winterhill_resample_clean.csv") as f:
...
results = []
total = len(samples)
print("Routing " + str(total) + " points to Winter Hill...\n")
...
with open("winterhill_resample_walks.json", "w") as f:
    json.dump(results, f, indent=2)

failed = [r for r in results if r["walk_mi"] is None]
print("\nDone. Total: " + str(len(results)) + ", Failed: " + str(len(failed)))
print("Upload winterhill_resample_walks.json to Claude")
