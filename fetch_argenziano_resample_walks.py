"""
Walking distances for the 578 high-resolution Argenziano zone sample
points.

Run: python3 fetch_argenziano_resample_walks.py
Outputs: argenziano_resample_walks.json -- upload to Claude
"""
...
samples = []
with open("argenziano_resample_v2_geocoded.csv") as f:
...
results = []
total = len(samples)
print("Routing " + str(total) + " points to Argenziano...\n")
...
with open("argenziano_resample_walks.json", "w") as f:
    json.dump(results, f, indent=2)

failed = [r for r in results if r["walk_mi"] is None]
print("\nDone. Total: " + str(len(results)) + ", Failed: " + str(len(failed)))
print("Upload argenziano_resample_walks.json to Claude")
